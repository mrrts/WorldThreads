...
validates_methods:
  ...
  - dense-curated-phrase-vs-discrete-list-rubric  # FIRST empirical comparison between the two count-with-thresholds sub-shapes. The 18:57 dense-phrase re-grade (same 12 samples, $0.0018) settled the trade-off: discrete list catches density (RI cells 0.83/1.00 partial-bite signal); dense phrase collapses multi-surface-element metaphors into single instances and catches novelty (RI cells 1.00/1.00, but caught "a man can come back to himself" as recovery-from-strain instance the list missed). Different instruments, different unit-of-measurement. Choice rule codified in CLAUDE.md: density measurement → list; presence/at-all measurement → dense; both at $0.002 each if the design needs both.
