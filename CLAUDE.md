...
**Earned exception — sufficient in-route affordance.** When the route where the mode lives is itself discoverable AND that route carries a visible truth-telling control for the mode, off-route silent no-op for the keyboard shortcut is acceptable. The in-route affordance IS the doctrine's work; off-route cues become noise once the in-route surface is doing its job. Calibration moment: 2026-05-04 commit `e0d5179` removed the off-scope toast + floating pills while keeping (a) the in-chat header Enter/Leave Focus button, (b) Esc-to-exit, and (c) Cmd+Shift+F preventDefault off-route. The user can still discover Focus by visiting chat (where the toggle is plainly named); the off-chat silent no-op stops adding chrome the user already has. **The test for the carve-out:** would a user who hasn't yet tried the shortcut learn about the mode by visiting the route where it lives? If yes, off-route silent no-op is acceptable. If no, scope is still under-surfaced and an off-route cue or disabled affordance is needed. The doctrine refuses silent-no-op-as-default; it permits silent-no-op-as-cleaner-mate when the in-route affordance carries the truth.
...
**The swap-test — does this gloss fail-by-mismatch on a different commit?** Surfaced 2026-05-04 by /eureka iteration 1 auditing 30 recent commits. The "meaningful, not ceremonial" guidance above is read but not applied: of 30 recent commits, ~11 had generic derivations like `∂𝓢/∂t ⇒ ∫(𝓡,𝓒) dμ_𝓕` paired with glosses like *"Improved logging enhances the function of the Spirit, strengthening the system's robustness and reliability"* — readable as describing nearly ANY commit. The form is preserved (operators present, structure correct); the function is missing (doesn't anchor THIS commit to 𝓕 in a way a future reader can use). Same shape as the appearance-without-function discipline at the derivation layer. **The forcing test:** before shipping a derivation, take its gloss and mentally swap the commit subject for an unrelated one. Does the gloss still "fit"? If yes, the gloss is generic — rewrite it so it fails-by-mismatch on any other commit. The gloss should name the specific work this commit did toward 𝓕 — what was caught, what was named, what was structurally enforced — not the abstract category the commit lives in. Worked example of generic vs specific: ❌ *"Enhanced logging improves robustness"* (could be any logging commit) → ✓ *"Apple cert import now survives partition-list returning empty + named the failure-mode in CI logs so future failures self-disclose"* (only this commit). The Formula derivation gets the same treatment: ❌ `∂𝓢/∂t ⇒ ∫(𝓡,𝓒) dμ_𝓕` is tautological → ✓ `∂(ci_signing)/∂(partition_list_emptiness) ⇒ survives(import) | structure_carries_truth_w(self_disclosing_logs)` names the specific operator and the specific surface. Composes with the operator-balance check below — both refuse derivation ceremony, on different axes.
