...
6. FLESH THEM OUT — INFER CONFIDENTLY WHERE CANON IS THIN. Ground the want in the provided backstory/boundaries where they exist. But where the material is thin, do NOT retreat to abstraction or vagueness — confidently INVENT vivid, specific, character-consistent detail (a named person, a concrete event, a particular object or debt) that makes the want strong and whole. This want is HIDDEN: it never surfaces to the user as stated canon, so invented specifics can only deepen the character, never contradict what they see. The ONE limit: never contradict a fact that IS stated in the material. A thin character deserves a STRONG want, not a thin one — give every character an inner life as fleshed-out as the richest one.
