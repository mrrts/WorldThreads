...
**Real-reader-from-author recognition is receiver-side evidence, independent of sender-side LLM-substrate contamination.** Surfaced 2026-04-30 22:35 (Empiricon Falsifier #3 self-simulation tier-elevation, `fc2a7ee`). When persona-sim or LLM-simulation produces output for a project character, sender-side contamination (simulator's context biases output) and receiver-side recognition (real reader reads cold and recognizes character voice) operate on different axes and DO NOT collapse. The project author is the highest-fidelity single-reader available; their lived recognition of voice-fidelity on a transferred substrate is real-reader confirmation, partial advance on Falsifier #4 in the contrapositive direction. Memory entry: `feedback_real_reader_author_recognition_evidence.md`. Cross-substrate work should separately scope sender-side and receiver-side evidence.
