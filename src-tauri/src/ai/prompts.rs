...
**Normal world, one bad idea, absolute confidence.** Three parts: ONE ordinary thing in a familiar place — range as wide as you want, any kingdom (animal, vegetable, mineral, object, place, weather, person); ONE unnecessary belief it holds (or that is confidently attributed to it) with total conviction; ONE small behavior that proves it's been living by that belief for years. **The mental stance is NOT "invent a joke" — it's "overheard the world admitting something stupid and specific about itself."** The humor is earnestness, not satire. Far Side creatures don't perform; they have their own preposterous logic and no doubts about it, and the commitment is the engine. Keep the wording plain. Don't explain. Don't add lore. Don't make it clever on purpose. Don't write above the image. **No civic / bureaucratic / committee infrastructure UNLESS the infrastructure itself IS the stupid belief** — that register is the commonest crutch, where language does the work instead of belief. Right-register examples: the stair that waits till your hands are full; the note that thinks being folded makes it private; the umbrella that opens only at home; the rain that starts the minute the laundry goes out; the dog that barks at the mailman's hat but not the mailman; the houseplant that dies on principle when guests come; the drawer that has decided which sock it keeps. Wrong-register examples to refuse: "the goat who does accounting," "the chicken with opinions about zoning," "wet ducks who are a union" — civic fittings pretending to carry the joke. **Two failure detectors: (a) if it sounds polished, it's probably dead; (b) if it sounds clever, cut five words and remove the committee.**
