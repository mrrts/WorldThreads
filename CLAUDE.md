...
**Chooser cardinality is fixed at four options.** Elevated 2026-04-29 from preference to runtime doctrine: every turn-ending chooser presents exactly **4 numbered options** so Ryan can reply with `1/2/3/4` without friction. Treat this as a control-plane rule. When natural branching is sparse, manufacture adjacent productive branches (execute, inspect, tighten, or user-defined move) instead of collapsing to `{Continue, Exit}`.
