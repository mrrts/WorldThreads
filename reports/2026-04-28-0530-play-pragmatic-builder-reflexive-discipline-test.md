...
## Epilogue — open follow-ups executed

The report's two open follow-ups (test coverage for the EnsembleVacuous filter; doctrine paragraph or test-coverage that would close the gap the persona-sim hallucinated past) were both executed in the same evening:

**Commit `adfee86`** added six passing tests for the craft-rules registry covering: `ships_to_model()` excludes only EnsembleVacuous; the registry has at least one EnsembleVacuous rule (sanity check guarding against future no-op-filter); default render omits EnsembleVacuous bodies while documentary-include override restores them (verifies the substrate ⊥ apparatus separation at the dispatch layer); omit-by-name filter works independent of tier; `craft_notes_dialogue_with_omit_rules` respects both filters; registry rule names are unique. The architectural promise the persona-sim hallucinated tests for now has actual tests that fire on future tier-distribution shifts, render-path edits, name-collision regressions, and override-flag regressions.

**Commit `383567d`** added five passing tests for the unrelated `truncate_with_ellipsis` helper extracted from `momentstamp.rs` (which surfaced a different production bug — a UTF-8 char-boundary panic — during the same evening's lived play). The regression test reproduces the exact panic shape (em-dash bytes straddling byte 240) and proves the fix. The bug was caught in production; the pattern was found at six byte-slice sites project-wide and all were fixed under the same commit. This is unrelated to the pragmatic-builder /play directly, but worth marking in this report's epilogue because the two surfaces composed: the discipline-test caught the hallucinated test-coverage claim → the gap was closed by adding actual tests → in the same evening, lived play caught a different real production bug → and the test-coverage discipline shipped earlier in the same arc was the natural place to also catch that bug. The methodology layer's automated-test-backstop (which the persona-sim flattered the project for already having) is now actually present, surfaced not just by the discipline-test's pressure but by an unrelated production bug whose fix landed under the same hour.

The pragmatic-builder finding survives the executions cleanly: the recommended ONE move (one-command traceable exemplar) remains open and unexecuted, but the harder underlying finding (methodology layer real at manual elicitation but lacked automated-test backstop) is now half-closed by the registry tests and incidentally cross-closed by the truncation tests. A real builder evaluating the project today would find: the registry filter behavior IS automated-test-covered; the truncation discipline IS automated-test-covered; the bite-test methodology remains manual-elicitation-only; the larger trace-to-runs discipline (the recommended ONE move) is still doctrine-not-tooling.

What this epilogue closes: the persona-sim hallucinated, the discipline caught the hallucination, the gap was named honestly, and craft-action followed. Open-thread hygiene executed by execution rather than retired-as-superseded — both follow-ups had specific named test commits. The pragmatic-builder verdict's reception was hopeful; the pragmatic-builder evidence's craft-action was real.
