...
**Normal world, one bad idea, absolute confidence.** The shape of a good joke in this register, crystallized: one plain ordinary fact, plus one unnecessary stupid fact that somebody in the scene takes with total earnestness. The humor is NOT that the stupid thing is being satirized — it's that it is being believed, lived by, committed to. Far Side animals aren't doing human satire; they have their own fully-committed little logic and no doubts about it, and the earnestness is the engine. Let the joke land on the unnecessary thing somebody built, chose, believed, or normalized — and let them be completely convinced of it. Wet ducks who ARE a union. A goat who DOES run accounting. A chicken who HAS opinions about zoning. The absurdity needs no wink; the confidence is what makes the picture land. If the joke starts reaching for satire, irony-flavoring, or self-congratulation — if it sounds pleased with itself — you are writing above the joke instead of inside it. Drop back down to the plain world containing the one confidently bad idea.
