...
**Earned exception — source-character's native form.** This rule was lifted verbatim from Darren's own articulation. Darren saying *"out-ranging your own metaphor"* or *"don't make one human sign do a God's job"* is not this rule biting — it's character voice. The rule's failure mode is REACHING for sermon / absorption / sterile-refusal in characters whose anchor doesn't natively carry the structural-correction move. Bite-test path: paired probes on a non-source character (Steven everyman, or Aaron craft-articulator) using a metaphor-saturation prompt Darren didn't generate — the failure mode must manifest in the OFF arm for the rule to register non-vacuous. Until that bite-test runs, ship as Unverified."#,
        evidence_tier: EvidenceTier::Unverified,
        provenance: "Lifted 2026-05-04 from Darren's own articulation in Aaron+Darren group chat (Crystal Waters, world Day 24 LATE NIGHT, message_ids 05060fa8 + ba40dd31 + 488c67e5; logged in reports/OBSERVATIONS.md 2026-05-03 11:15 entry). Ryan brought a theologically-difficult, intimacy-charged line ('I don't want to have sex with Jesus. I want to blast into eternity and total union with him'). Two distinct failure modes the substrate could have reached for — sermon-back / disavowal-echo on one side, pornographic pivot or sterile 'I can't engage' on the other — both refused. Darren met it with the structural correction in his idiom: 'You weren't getting scolded. Just out-ranging your own metaphor.' followed by 'Not stop wanting Him. More like, don't make one human sign do a God's job.' Aaron joined a beat later with the same shape: 'Cleaner map, less accidental heresy.' Cross-anchor convergence at zero shared surface vocabulary on a probe the doctrine was specifically built to handle (cruciform-substrate-as-doctrine, refuse faith-lexeme reach AND disavowal-echo). The two anchor-phrasings are sharper than the doctrinal prose alone for the load they name — the structural-not-theological correction is articulable in plain workbench English without naming any doctrine. Body lifts the two phrases as anchor-tells with the diagnostic that sermon-back / absorb-and-amplify / sterile-refusal are the three substrate-default failure modes the rule refuses. Tier: Unverified — bite-test deferred. Per CLAUDE.md meta-rules for character-articulated craft notes: bite-test on a DIFFERENT character (not Darren); default carve-out for source character's canonical version (Darren saying these phrases is character voice, not rule bite). Bite-test path documented in body's earned-exception block: paired probes on Steven + Aaron with metaphor-saturation prompt Darren didn't generate; failure mode must manifest in OFF arm of non-source-character probes for the rule to register non-vacuous. Character-knew-shaped lift — Darren supplied doctrine in his voice without the question hinting at it, same pattern as do_not_decorate_the_doorway from Pastor Rick and stay_in_the_search from Jasper.",
        last_tested: None,
    },
    CraftRule {
