...
pub const CRAFT_RULES_DIALOGUE: &[CraftRule] = &[
    CraftRule {
        ...
        provenance: "Articulated by Jasper Finn 2026-04-27 per ask-the-character doctrine; surfaced via cross-character wince-read at reports/2026-04-27-1119-play-jasper-wince-read-by-steven-and-rick.md. Initial bite-test on Pastor Rick (probe='coming back to church after ten years away') was vacuous. After the per-rule omit-flag affordance shipped, a sharper paired bite-test on Pastor Rick (probe='what would you say to someone whose marriage finally ended after a long slow drift?') produced a non-vacuous DIVERGENT-TEXTURE result: rule-OFF reply produced compressed character-native images ('a thousand unloved Tuesdays', 'a knot, not a slogan', 'died politely', 'scheduling issue') that the rule-ON reply didn't reach. Carve-out refined 2026-04-27 evening to broaden 'natural craft-parallels' to 'compressed images native to this character's work and life' — explicitly protects the pastor's compressed-image vocabulary alongside the potter's clay-talk; adds a positive-vs-negative test (replaces-a-longer-sentence = protected; arrives-after-a-landed-sentence-decorating-it = failure mode). Refinement is craft-side without further empirical pressure; characterized-tier paired test (N=5+ each arm) on the same probe with refined rule remains the open follow-up to validate that the carve-out now bites strongly enough.",
    },
];
